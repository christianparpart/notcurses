...
int redraw_plot_##T(nc##X##plot* ncp){ \
  ...
  const int scale = ncp->bset->geom == NCBLIT_PIXEL ? \
      ncplane_notcurses_const(ncp->ncp)->tcache.cellpixx : ncp->bset->width; \
  int dimy, dimx; \
  ncplane_dim_yx(ncp->ncp, &dimy, &dimx); \
  const int scaleddim = dimx * scale; \
  /* each transition is worth this much change in value */ \
  const size_t states = (ncp->bset->geom == NCBLIT_PIXEL ? \
      ncplane_notcurses_const(ncp->ncp)->tcache.cellpixy : ncp->bset->height) + 1; \
